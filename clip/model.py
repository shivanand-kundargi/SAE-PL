...
class VisionTransformer_MaPLe(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        # Hook storage for SAE integration
        self._hooked_activations = None
        self._hook_handle = None

    def register_sae_hook(self, target_layer):
        """Register a forward hook on a specific transformer layer to capture activations."""
        ...
        if target_layer < 0:
            target_layer = num_layers + target_layer

        def hook_fn(module, input, output):
            # output is [x, compound_prompts_deeper, counter]
            # x is in LND format (seq_len, batch, dim)
            self._hooked_activations = output[0].permute(1, 0, 2).detach()  # -> NLD (batch, seq_len, dim)

        self._hook_handle = self.transformer.resblocks[target_layer].register_forward_hook(hook_fn)
