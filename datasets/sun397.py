    ...
    def __init__(self, cfg):
        ...
        if not os.path.isdir(self.image_dir):
            raise FileNotFoundError(
                f"SUN397 images not found at {self.image_dir}. "
                f"Expected a directory named 'SUN397' under DATASET.ROOT/sun397. "
                f"Run `bash multimodal-prompt-learning/scripts/download_datasets.sh` "
                f"or place/extract SUN397.tar.gz into {self.dataset_dir}."
            )
